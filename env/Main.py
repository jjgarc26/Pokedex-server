...
def get_pokemon(pokemon_name):
    ...
    return response
@app.route('/regions/<region_name>', methods=['GET'])

def get_region_pokedex(region_name):
    try:
        print('Getting pokemon information')
        request = RequestCall()
        data = request.get_region_information(region_name)
        response = app.response_class(
            response = json.dumps(data),
            status = 200,
            mimetype = 'application/json'
        ...
        response = app.response_class(
            response=f'error has occurred in "pokedex/{region_name}": {error}',
            status=400,
            mimetype='application/json'
        )
    return response